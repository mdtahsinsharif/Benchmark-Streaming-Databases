...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Nexmark q14 on Spark.")
    ...
    parser.add_argument("--query-name", default="nexmark_q14")
    ...
    parser.add_argument(
        "--output-path",
        default="/srv/nfs/spark/nexmark/normal/q14",
    )
...
def build_bid_projection(events_df):
    ...
    return (
        bid_df
        .withColumn("price", F.col("price") * F.lit(0.908))
        .withColumn(
            "bidTimeType",
            F.when((F.hour("dateTime") >= 8) & (F.hour("dateTime") <= 18), "dayTime")
             .when((F.hour("dateTime") <= 6) | (F.hour("dateTime") >= 20), "nightTime")
             .otherwise("otherTime"),
        )
        .withColumn("c_counts", count_char(F.col("extra"), F.lit("c")))
        .filter((F.col("price") > 1000000) & (F.col("price") < 50000000))
        .select(
            "auction",
            "bidder",
            "price",
...
def main() -> None:
    ...
    builder = SparkSession.builder.appName("nexmark_q14_latency")
